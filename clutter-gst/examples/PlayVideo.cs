...
public class VideoApp 
{
	...
	{
		vtexture.RotateY ((float)args.FrameNum * 12,
	...
	void HandleButtonPressEvent (object o, ButtonPressEventArgs args) 
	{
		...
		if (actor == control_seek1 
			...
			int y;
		 	...
		 	control_seekbar.GetAbsPosition (out x, out y);
	...
	public VideoApp (string filename)
	{
		...
		// panel
		Gdk.Pixbuf pixb = new Gdk.Pixbuf ("vid-panel.png");
		control_bg = new Texture (pixb);

		// play button
		pixb = new Gdk.Pixbuf ("media-actions-start.png");
		control_play = new Texture (pixb);

		// pause button
		pixb = new Gdk.Pixbuf ("media-actions-pause.png");
		control_pause = new Texture (pixb);
		...
		control.Add (control_bg);
		control.Add (control_play);
 
		control.Add (control_pause);
		control.Add (control_seek1);
		control.Add (control_seek2);
		control.Add (control_seekbar);
		control.Add (control_label);
