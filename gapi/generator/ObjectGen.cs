	...
	public class ObjectGen : ObjectBase  {
		...
		void GenClassMembers (GenerationInfo gen_info, string cs_parent)
		{
			...
			if (cs_parent == "")
				...
				sw.WriteLine ("\t\tstatic uint class_offset = GTypeExtensions.GetClassSize (((GLib.GType) typeof ({0})));", cs_parent);
			...
			sw.WriteLine ("\t\t\telse {");
			sw.WriteLine ("\t\t\t\tIntPtr class_ptr = new IntPtr (GTypeExtensions.GetClassPtr (gtype).ToInt64 () + class_offset);");
